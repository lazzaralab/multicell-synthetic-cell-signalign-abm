...
;store starting radius of spheroid
  ...
  if adhesion = "core-pole" [set homotypic-prob-c 100 set homotypic-prob-d 100 set heterotypic-prob 0]
  if adhesion = "core-shell-green" [set homotypic-prob-c 100 set homotypic-prob-d 0 set heterotypic-prob 100]
  if adhesion = "core-shell-red" [set homotypic-prob-c 0 set homotypic-prob-d 100 set heterotypic-prob 100]
...
;initialize A-type and B-type cells in random locations of the spheroid
    ...
    print(spheroid-radius-b)
      ...
      sprout-as 1[
        ...
        if signaling = "no-signaling"[
          set color red
          set d-status true
          set r-color true
        ]
        if signaling = "uni-directional"[
          set color red
          set d-status true
          set r-color true
        ]
        if signaling = "bi-directional"[
          set color blue
          set d-status false
          set r-color false
        ]
  ...
  if time-step = 15[
    set run-time 200
    set cluster-pause-delay 6
    set c-express-delay 20
    set c-express-thresh 20
    set d-express-delay 40
    set d-express-thresh 20
  ]
  if time-step = 30[
    set run-time 100
    set cluster-pause-delay 3
    set c-express-delay 10
    set c-express-thresh 10
    set d-express-delay 20
    set d-express-thresh 10
  ]
  if time-step = 60[
    set run-time 50
    set cluster-pause-delay 1
    set c-express-delay 5
    set c-express-thresh 5
    set d-express-delay 10
    set d-express-thresh 5
  ]
...
;update location space of spheroid
  let spheroid-thresh run-time / 4
 if ticks < run-time and ticks mod spheroid-thresh = 0 [set spheroid-radius spheroid-radius - spheroid-radius-inc ask patch 0 0 [set viable-patches patches in-radius spheroid-radius]]
...
;  let move-chance random 100
  ask turtles[set num-moves 0]
;find number of clusters in spheroid and iteratively move each cluster
  ask turtles [ifelse breed = bs[cluster-c] [cluster-d]]
  ...
  ifelse signaling = "no-signaling"[
  ask turtles with [count in-link-neighbors = 0][
      move-cen
      if breed = as [if random 100 <  homotypic-prob-d [probe-surroundings]]
      if breed = bs [if random 100 < homotypic-prob-c [probe-surroundings]]
  ]
  ]
  [
...
;    if move-chance < 70[
      move-cen
    ifelse signaling = "uni-directional"[][if ticks > (d-express-delay) [if r-color = false [express-d]]]
...
to move
  ...
  if any? viable-empty-spots [
    ...
    if breed = as [ if d-status = true [set cell-move-count-mCherry cell-move-count-mCherry + 1]]
    if breed = bs [ if c-status = true [set cell-move-count-GFP cell-move-count-GFP + 1]]
...
to express-c
  ...
  let c 0
  if signaling = "no-signaling"[
    set c count as with [r-color = false] in-radius 1
  ]
  if signaling = "uni-directional"[
    set c count as with [r-color = true] in-radius 1
  ]
  if signaling = "bi-directional"[
    set c count as with [r-color = false] in-radius 1
  ]
...
to switch-spots [x y]
  ...
  ask x [move-to patch2 set num-moves num-moves + 1]
  ask y [move-to patch1 set num-moves num-moves + 1]
  ask links[tie]
end
...
to setOutputFile
  ...
  set output_folder (word "Output_3.1/Cell_Migration_Analysis/"a-ratio"A|"b-ratio"B_SphRad-"original-spheroid-radius"/")

;  set output_folder (word "Output_3.1/Signaling_Adhesion_Tests/"signaling"_"adhesion"/")
...
to outputIndivCellMovement
  file-close-all
  file-open outputFilename
  file-print (word "tick: "ticks)

  let total-blue-moves 0
  let total-gray-moves 0
  let total-green-moves 0
  let total-red-moves 0

  if any? as with [d-status = false][
    ask as with [d-status = false][set total-blue-moves total-blue-moves + num-moves]
  ]
  if any? bs with [c-status = false][
    ask bs with [c-status = false][set total-gray-moves total-gray-moves + num-moves]
  ]
  if any? bs with [c-status = true][
    ask bs with [c-status = true][set total-green-moves total-green-moves + num-moves]
  ]
  if any? as with [d-status = true][
    ask as with [d-status = true][set total-red-moves total-red-moves + num-moves]
  ]

  file-write "blue"
  file-write total-blue-moves / (count as with [d-status = false] + 1e-10)
  file-print ""

  file-write "gray"
  file-write total-gray-moves / (count bs with [c-status = false] + 1e-10)
  file-print ""

  file-write "green"
  file-write total-green-moves / (count bs with [c-status = true] + 1e-10)
  file-print ""

  file-write "red"
  file-write total-red-moves / (count as with [d-status = true] + 1e-10)
  file-print ""
...
signaling
signaling
"no-signaling" "uni-directional" "bi-directional"
2
...
adhesion
adhesion
"core-pole" "core-shell-green" "core-shell-red" "test"
0
...
<experiments>
  ...
  <experiment name="1DSA_cluster-pause-delay [2 3 4]" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="1DSA_cluster-move-size [4 5 6]" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="1DSA_c-express-thresh [9 10 11]" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="1DSA_d-express-thresh [9 10 11]" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="1DSA_d-express-delay [18 20 22]" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="1DSA_exp-expression-const [0.18 0.2 0.22]" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="1DSA_heterotypic-prob [0 5 10]" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="1DSA_homotypic-prob [80 90 100]" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="1DSA_c-express-delay [9 10 11]" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="SignalingRemovedExperiment" repetitions="150" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="300"/>
  ...
  <experiment name="WithSignalingExperiment" repetitions="150" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="300"/>
  ...
  <experiment name="RulesetTests" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
    ...
    <enumeratedValueSet variable="signaling">
      <value value="&quot;no-signaling&quot;"/>
      <value value="&quot;uni-directional&quot;"/>
      <value value="&quot;bi-directional&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="a-ratio">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="adhesion">
      <value value="&quot;core-pole&quot;"/>
      <value value="&quot;core-shell-green&quot;"/>
      <value value="&quot;core-shell-red&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="d-express-delay">
      <value value="20"/>
    </enumeratedValueSet>
  ...
  <experiment name="SphRad6.4_HomoHetero" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="300"/>
  ...
  <experiment name="2500HPAF:2500MiaPacaExample" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="2500HPAF:250MiaPacaExample" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
  ...
  <experiment name="TimeStepSensitivityAnalysis" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <exitCondition>run-complete</exitCondition>
    <enumeratedValueSet variable="percent-saturation">
      <value value="0.65"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="exp-expression-const-c">
  ...
  <experiment name="CellMigrationAnalysis" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="100"/>
    <enumeratedValueSet variable="percent-saturation">
